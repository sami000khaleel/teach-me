...
def store_image(image):
    try:
        ...
        index = names.index(f"{student_id}.jpg") if f"{student_id}.jpg" in names else (names.index(f"{student_id}.png") if f"{student_id}.png" in names else -1)
        ...
        if file and file.filename.endswith('.jpg') or file and file.filename.endswith('.png'):
            ...
            if result == "True":
                ...
                if state == "True":
                    ...
                    return jsonify({'result': name , 'message': action}), 200
                else :
                    return jsonify({'message': 'facesDoNotMatch'}), 200
            else:
                return jsonify({'message': 'noFace'}), 200
